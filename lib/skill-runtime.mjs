...
const CORE_COMPATIBILITY_EVENTS = [
  "session.complete",
  "order.succeeded",
  "order.failed",
  "refund.succeeded",
...
function isDocsOnlyQuestion(input) {
  const haystack = normalize(input);
  return (
    /\b(do not|don'?t|dont)\s+write\s+code\b/.test(haystack) ||
    /\bwithout\s+writing\s+code\b/.test(haystack) ||
    /\bno\s+(implementation\s+)?code\s+(sample|samples|example|examples)\b/.test(haystack) ||
    /\b(explain|docs|documentation)\s+only\b/.test(haystack) ||
...
function isNonClinkPaymentPrompt(input) {
  ...
  const haystack = normalize(input);
  const mentionsOtherPsp =
    /\b(stripe|paypal|adyen|braintree|square)\b/.test(haystack) ||
    /\bcheckout\.com\b/.test(raw) ||
    /\bcheckout\s+com\b/.test(haystack);
...
export function detectRoute({ prompt, contextBlocks = [] }) {
  const combinedText = [prompt, ...contextBlocks.map((item) => item.content || item)].join("\n");
  const haystack = normalize(combinedText);
  const signals = getRouteSignals(haystack);

  if (isNonClinkPaymentPrompt(combinedText)) {
    return "none";
  }
...
function includesAnySignal(haystack, signals) {
  return signals.some((signal) => haystack.includes(normalize(signal)));
}

function getWebhookEventRecommendation(input, route) {
  const haystack = normalize(input);
  const compatibilityOnly = includesAnySignal(haystack, [
    "compatibility-only",
    "compatibility only",
  ...
  const subscriptionSignals = includesAnySignal(haystack, [
    "subscription",
    "recurring billing",
    "recurring payment",
    "renewal",
  ...
  const oneTimeSignals = includesAnySignal(haystack, [
    "one-time checkout",
    "one time checkout",
    "one-time payment",
    "one time payment",
  ...
  const checkoutLifecycleSignals = includesAnySignal(haystack, [
    "checkout",
    "session.expired",
    "session expired",
    "order",
  ...
  const paymentMethodSignals = includesAnySignal(haystack, [
    "payment method",
    "payment methods",
    "payment_method",
    "saved card",
  ...
  const lifecycleSignalCount = [
    ["renewal", "续费"],
    ["subscription.cancelled", "cancel subscription", "取消订阅"],
    ["past_due", "past due", "dunning", "催缴", "逾期"],
    ["entitlement", "access revocation", "权限回收", "权益回收"],
    ["dispute", "拒付"],
    ["refund", "退款"],
  ].filter((signals) => includesAnySignal(haystack, signals)).length;
  const completeCommerceContext = includesAnySignal(haystack, [
    "complete billing",
    "full billing",
    ...
    "生产集成",
    "生产级收费",
  ]) || lifecycleSignalCount >= 3;

  const productionCommerceContext = /\bproduction\b/.test(haystack) || includesAnySignal(haystack, [
    "go live",
    "上生产",
    "生产环境",
    "生产集成",
  ...
  if (completeCommerceContext || productionCommerceContext) {
    return { events: "commerce", compatibilityOnly: false };
  }

  const pureCompatibilityContext = compatibilityOnly &&
    !subscriptionSignals &&
    !oneTimeSignals &&
    !checkoutLifecycleSignals &&
    !paymentMethodSignals &&
    lifecycleSignalCount === 0;
  if (pureCompatibilityContext) {
    return {
      events: "core",
      compatibilityOnly: true,
      coreEvents: [...CORE_COMPATIBILITY_EVENTS],
      omissionRisks: [...CORE_OMISSION_RISKS],
    };
  }

  if (subscriptionSignals) {
    const events = ["checkout", "subscriptions", "disputes"];
    if (paymentMethodSignals) events.push("payment-methods");
    return { events: events.join(","), compatibilityOnly: false };
  }

  if (oneTimeSignals || checkoutLifecycleSignals) {
    const events = ["checkout", "disputes"];
    if (paymentMethodSignals) events.push("payment-methods");
    return { events: events.join(","), compatibilityOnly: false };
  }

  if (route === "merchant_new_user_onboarding") {
    return { events: "checkout,disputes", compatibilityOnly: false };
  }

  return { events: "commerce", compatibilityOnly: false };
}

function buildWebhookEndpointCommand(recommendation) {
  return `clink webhook endpoint ensure --url <public-webhook-url> --events ${recommendation.events} --save-secret --json`;
}

function buildWebhookEndpointArtifact(input, route, lead = "Configure the public HTTPS webhook with") {
  const recommendation = getWebhookEventRecommendation(input, route);
  const command = buildWebhookEndpointCommand(recommendation);
  const coreWarning = recommendation.compatibilityOnly
    ? ` Compatibility/minimal-demo only: core contains exactly 6 events (${recommendation.coreEvents.join(", ")}). It omits lifecycle coverage for ${recommendation.omissionRisks.join(", ")}; use --events commerce for complete billing, subscription, or production integration.`
    : "";
  return buildArtifact(
    "webhook_endpoint_automation",
    null,
    `${lead} ${command}.${coreWarning}`,
    {
      command,
      events: recommendation.events,
      compatibility_only: recommendation.compatibilityOnly,
      ...(recommendation.compatibilityOnly
        ? {
            core_events: recommendation.coreEvents,
            omitted_lifecycle_risks: recommendation.omissionRisks,
          }
        : {}),
    }
  );
...
function shouldEmitWebhookEndpointArtifact(route, input) {
  if (["merchant_standard_integration", "merchant_new_user_onboarding"].includes(route)) return true;
  if (!["review", "integration_validation"].includes(route)) return false;
  const haystack = normalize(input);
  return includesAnySignal(haystack, [
    "webhook",
    "billing",
    "payment",
    "checkout",
...
export function buildArtifacts({
  ...
  const webhookEndpointArtifact = buildWebhookEndpointArtifact(combinedText, route);
  const includeWebhookEndpoint = shouldEmitWebhookEndpointArtifact(route, combinedText);

  if (policy.remediationOnly) {
    const remediationSummary = includeWebhookEndpoint
      ? `List the exact fixes required before retrying production promotion, including ${webhookEndpointArtifact.command}`
      : "List the exact fixes required before retrying production promotion";
    return [
      buildArtifact("validation_report", null, "Summarize failed production checks and unresolved validation gaps"),
      buildArtifact("remediation_checklist", null, remediationSummary, includeWebhookEndpoint
        ? { command: webhookEndpointArtifact.command, events: webhookEndpointArtifact.events }
        : {}),
    ];
  }
  ...
  if (route === "merchant_standard_integration") {
    const artifacts = [
      ...
      buildArtifact("cli_capability_checklist", null, "Verify the offline bundled clink-integ-cli and confirm env, auth secret set, api request, catalog import, and webhook endpoint ensure are available"),
  ...
  if (route === "merchant_new_user_onboarding") {
    return [
      ...
      buildArtifact("secret_setup_checklist", null, "Offline bundled CLI + optional preprovisioned Playwright local clink login bootstrap or browserless manual Secret Key setup, clink auth secret set, webhook endpoint ensure, signing-secret sync, and safe storage placeholders"),
      buildArtifact("cli_setup_checklist", null, "Use the offline bundled clink-integ-cli and verify env, auth secret set, api request, catalog import, and webhook endpoint ensure"),
      ...
      buildWebhookEndpointArtifact(combinedText, route, "After the webhook route has a public HTTPS URL, use"),
  ...
  if (route === "review") {
    ...
    if (includeWebhookEndpoint) artifacts.push(webhookEndpointArtifact);
    return artifacts;
  }
...
export async function runSkillRuntime({
  ...
  if (route === "merchant_standard_integration") {
    const webhookRecommendation = getWebhookEventRecommendation(combinedText, route);
    const webhookCommand = buildWebhookEndpointCommand(webhookRecommendation);
    ...
    runtimeState = appendRuntimeNote(
      runtimeState,
      `Prefer ${webhookCommand} for endpoint management, then sync CLINK_WEBHOOK_SIGNING_KEY into the app runtime and restart or redeploy.`
    );
    if (webhookRecommendation.compatibilityOnly) {
      runtimeState = appendRuntimeNote(
        runtimeState,
        `The core preset is compatibility/minimal-demo only and contains exactly 6 events (${webhookRecommendation.coreEvents.join(", ")}); it omits ${webhookRecommendation.omissionRisks.join(", ")}. Use --events commerce for complete billing, subscription, or production integration.`
      );
    }
    if (shouldAddSandboxCardBindingTestNote({ route, runtimeState })) {
      runtimeState = appendRuntimeNote(
        runtimeState,
        "After sandbox integration is ready, card-binding payment tests can use test card 4242424242424242 with any 3-digit CVC and any future expiry date; do not use this guidance for production or claim payment success before a real sandbox test payment and merchant fulfillment checks complete."
      );
    }
    if (hasElementsSignals(combinedText)) {
      runtimeState = appendRuntimeNote(
